...
def solve2(grid: list[list[str]], start_pos: tuple[int, int], end_pos: tuple[int, int]):
  visited = [[False] * len(grid[0]) for _ in range(len(grid))] # we techincally don't need this since we can use distance_to_end as a visited array as well
  ...
  while queue:
    *coords, dist = queue.popleft()
    ...
    for delta in [(0, -1), (0, 1), (-1, 0), (1, 0)]:
      ...
      if inb(*new_pos) and not visited[new_pos[0]][new_pos[1]] and not is_wall(*new_pos):
    ...
    for cheat_time in range(2, 21):
      for cheat_delta in full_cheat_deltas[cheat_time]:
        ...
        if inb(*cheat_pos) and not is_wall(*cheat_pos):
          ...
          time_saved = distance_to_end[coords[0]][coords[1]] - distance_to_end[cheat_pos[0]][cheat_pos[1]] - cheat_time
          if time_saved > 0:
  ...
  while queue:
    ...
    for cd1 in udlr:
      for cd2  in udlr:
        ...
        if inb(*cheat_pos) and not is_wall(*cheat_pos):
          # Calculate the distance saved as cur - new - 2 (since each cheat takes 2 seconds)
          time_saved = distance_to_end[coords[0]][coords[1]] - distance_to_end[cheat_pos[0]][cheat_pos[1]] - 2
          if time_saved > 0:
            if time_saved in time_saves:
